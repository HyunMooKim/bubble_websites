...
설멍: MBTI 기반 심리 검사를 한 후, 각자에게 맞는 유형의 할로윈 코스튬을 추천해주는 사이트.

데이터 타입 - 딱히 복잡하지 않다. 유저로그{ E/I, S/N, T/F, J/P , MBTI결과, 슬러그 }  이정도가 핵심이다. 
슬러그는 각자 결과 페이지를 공유할 수 있도록 고유 url을 만들기 위함이다.

구조: 누군가 페이지에 들어오면 '유저' 데이터타입을 생성해 답변을 기록한다.
...
예를 들어 E or I 를 결정짓는 질문을 제시하고, E에 해당하면 유저의 E/I 데이터에 + 0(사실상 아무것도 안변함) , I에 해당하면 + 1 을 한다.
